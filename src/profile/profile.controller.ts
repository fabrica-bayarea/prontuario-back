...
export class ProfileController {
  ...
  @ApiOperation({
    summary: 'Operação de listar todos program',
    description: 'Operação de listar todos program que o usuário participa',
  })
  @ApiResponse({ status: 200, description: 'Programas listado com sucesso' })
  @ApiResponse({ status: 404, description: 'Perfil não encontrado' })
  @Get('program/')
  async getProgramsParticipate(@Request() req): Promise<Program[]> {
    const idUser = req.user.id;
